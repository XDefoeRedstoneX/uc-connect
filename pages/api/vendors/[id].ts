...
export default createHandler({
  GET: method({
    ...
    handler: async ({ req, supabase, res }) => {
      const { id } = req.query;
      if (typeof id !== "string") return res.status(400).json({ error: "Invalid vendor id" });
      ...
      if (vendorError) return res.status(404).json({ error: "Vendor not found" });
      ...
      if (metricsResult.error) return res.status(500).json({ error: "Unable to load vendor metrics" });
      if (hoursResult.error) return res.status(500).json({ error: "Unable to load vendor hours" });
      if (itemsResult.error) return res.status(500).json({ error: "Unable to load vendor items" });
