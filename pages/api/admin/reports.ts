...
export default createHandler({
  GET: method({
    ...
    handler: async ({ req, supabase, res }) => {
      ...
      if (error) return sendInternalServerError(res, "Gagal memuat laporan", error);

      // Enrich each report with a target_url + target_preview so admins can jump
      // straight to the offending row. Batched per target_type to keep this O(1)
      // extra calls.
      type ReportRow = {
        id: string;
      ...
      const [vendorRows, reviewRows, threadRows, replyRows] = await Promise.all([
        ...
        ids("thread").length
          ? supabase.from("forum_threads").select("id,title,forum_categories!inner(slug)").in("id", ids("thread"))
  ...
  PATCH: method({
    ...
    handler: async ({ supabase, userId, body, res }) => {
      const { report_id, status } = (body ?? {}) as { report_id?: string; status?: "resolved" | "dismissed" };
      ...
      if (error) {
        ...
        return sendInternalServerError(res, "Gagal memperbarui status laporan", error);
      }
  ...
  DELETE: method({
    ...
    handler: async ({ supabase, userId, body, res }) => {
      ...
      if (error) {
        ...
        return sendInternalServerError(res, "Gagal menghapus laporan", error);
      }
